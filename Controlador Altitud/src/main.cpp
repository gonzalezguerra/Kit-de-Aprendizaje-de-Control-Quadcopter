...
volatile int16_t g_setpoint_cm = 40; 
volatile uint8_t g_flags = 0; 
const float THROTTLE_HOVER = 0.45f;
...
void Task_AttitudeControl(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (g_flags & FLAG_ARMED) {
            writePulseUs(0, floatToPulseUs(clampf(motor1_out, 0.0f, 0.65f)));
            writePulseUs(1, floatToPulseUs(clampf(motor2_out, 0.0f, 0.65f)));
            writePulseUs(2, floatToPulseUs(clampf(motor3_out, 0.0f, 0.65f)));
            writePulseUs(3, floatToPulseUs(clampf(motor4_out, 0.0f, 0.65f)));
        } else {
...
void Task_AltitudeControl(void *pvParameters) {
    ...
    for (;;) {
        ...
        // Fusión y Limitación
        float altitud_local = clampf(tof_cm, 0.0f, 150.0f); // 1.5 metros max
        ...
        if (g_flags & FLAG_ARMED) {
            ...
            throttle_base_local = clampf(throttle_base_local, 0.0f, 0.65f);
            Serial.printf("Setpoint: %d cm, medida: %.1f cm, throttle_base: %.2f, (P: %.3f, I: %.3f, D: %.3f)\n",
...
void setup() {
   ...
   if(!mpu.begin()) { 
    ...
    // Dale tu taglio Goatcaz
    float alt_kp = 0.0065f; 
    float alt_ki = 0.017f;
    float alt_kd = 0.002f;

    altController->setGains(alt_kp, alt_ki, alt_kd);         // PID
    altController->setLimits(-0.0f, 0.2f);
    altController->setAntiWindup(-5.0f, 5.0f);
