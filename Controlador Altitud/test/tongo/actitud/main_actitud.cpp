...
static const uint16_t PULSE_MIN_US = 1100; // Minimo para que giren
static const uint16_t PULSE_MAX_US = 2100; // Maximo para que giren
...
void Task_AttitudeControl(void *pvParameters) {
    ...
    for (;;) {
        ...
        motor1_out = clampf(motor1_out, 0.0f, 0.65f);
        motor2_out = clampf(motor2_out, 0.0f, 0.65f);
        motor3_out = clampf(motor3_out, 0.0f, 0.65f);
        motor4_out = clampf(motor4_out, 0.0f, 0.65f);
